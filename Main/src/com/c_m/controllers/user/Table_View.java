package com.c_m.controllers.user;
...
import com.c_m.app.App;
import com.c_m.controllers.admin.ExportExcel;
import com.c_m.controllers.misc.SceneCtrl;
import com.c_m.database.Psql;
import com.c_m.modals.AlertModule;
import com.c_m.modals.MemberModel;
...
public class Table_View implements Initializable {
  ...
  public void initialize(URL location, ResourceBundle bundle) {
    ...
    Salvation.setCellValueFactory(
      new PropertyValueFactory<MemberModel, String>("Salvation")
    );
    Sex.setCellValueFactory(
      new PropertyValueFactory<MemberModel, String>("Sex")
    );
    waterBapt.setCellValueFactory(
      new PropertyValueFactory<MemberModel, String>("waterBapt")
    );
    SpiritBapt.setCellValueFactory(
      new PropertyValueFactory<MemberModel, String>("SpiritBapt")
    );
    Surbub.setCellValueFactory(
      new PropertyValueFactory<MemberModel, String>("Surbub")
    );
      ...
      while (res.next()) {
        loadList.add(
          new MemberModel(
            ...
            res.getString("id_no"),
            res.getString("children_num"),
            res.getString("maritial_status"),
            res.getString("date_joined"),
            res.getString("dob"),
            res.getString("address"),
            res.getString("surbub"),
            res.getString("home_phone"),
            res.getString("work_phone"),
            res.getString("mobile_phone"),
            res.getString("employer"),
            res.getString("position"),
            res.getString("email"),
            res.getString("home_group_leader"),
            res.getString("department_leader"),
            res.getString("department"),
            res.getString("salvation"),
            res.getString("water_baptism"),
            res.getString("spirit_baptism")
          )
        );
      }
      ...
      for (int y = 0; y < psqlTable.getColumns().size(); y++) {
        if (psqlTable.getColumns().get(y).getCellData(x) != null) {
          row
            .createCell(y)
            .setCellValue(
              psqlTable.getColumns().get(y).getCellData(x).toString()
            );
        } else {
